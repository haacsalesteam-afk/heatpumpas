...
def load_sheet_data(sheet_name):
    try:
        ...
        cols[20], cols[21], cols[22], cols[23] = "센서이상", "점검자", "검사 완료일", "비고(QM)"
        ...
        # 시운전 (AH~AO: 8칸) - UI 입력값 7개 + 공백 패딩 1개 연동
        cols[33], cols[34], cols[35], cols[36] = "가동시간", "시운전압력-저", "시운전압력-고", "시운전전류"
        cols[37], cols[38], cols[39], cols[40] = "물온도-부하", "물온도-열원", "비고(시운전)", "시운전_Extra"
        
        # 뒷부분 데이터 및 사진 (AP~AY)
        cols[41], cols[42], cols[43], cols[44], cols[45] = "사업명", "낙찰업체명", "대리점", "프로젝트번호", "제조프로젝트"
        cols[46], cols[47] = "제조오더", "SERVICE No." # AU, AV
        cols[48], cols[49], cols[50] = "QM사진", "설치사진", "시운전사진" # AW, AX, AY
                ...
                with st.form("testrun_form"):
                    ...
                    if submit_test:
                        with st.spinner("저장 중..."):
                            ...
                            # 오류방지용 데이터 8개 매핑 (AH~AO 구간) -> 마지막 공백문자 추가 
                            update_data = [safe_text(x) for x in [t_time, t_plow, t_phigh, t_amp, t_tload, t_theat, t_note, ""]]
                            for idx in sel_equips.index:
                                # 열 삽입 반영 범위 이동: AH:AO (8칸)
                                ws_equip.update(f"AH{c_df.loc[idx, 'row_index']}:AO{c_df.loc[idx, 'row_index']}", [update_data])
                                # 열 삽입 반영 시운전사진: AY
